 ...
 * @subpackage Admin
 */
if ( ! class_exists( 'Click_Theme_Info' ) ) {
    class Click_Theme_Info{
        ...
        public function at_theme_info_screen() {
            ...
            if (
                ...
                $notice_nag = get_option( 'click_admin_notice_welcome' );
                if ( ! $notice_nag ) {
                    echo '<div class="at-gsm-notice">
                        <small class="plugin-install-notice">'.esc_html__('Clicking the button below will install and activate the Advanced Import plugin.','click').'</small>
                        <a class="at-gsm-btn button button-primary button-hero" href="#" data-name="" data-slug="" aria-label="'.esc_html__('Get started with click','click').'">
                         '.esc_html__('Get started with click','click').'                   
                         </a>
                    </div>';
                }
        ...
        public function recommended_actions() {
            ...
            if ( ! empty( $recommended_actions ) ) {
                        ...
                        if ( ! empty( $plugin_slug ) ) {
                            ...
                            switch ( $active['needs'] ) {
                                ...
                                case 'install':
                                    ...
                                    $label = esc_html__( 'Install', 'click' );
                                    break;

                                case 'activate':
                                    $class = 'activate-now button button-primary';
                                    $label = esc_html__( 'Activate', 'click' );

                                    break;

                                case 'deactivate':
                                    $class = 'deactivate-now button';
                                    $label = esc_html__( 'Deactivate', 'click' );
                ...
                if ( $hooray ){
                    echo '<span class="hooray">' . esc_html__( 'Hooray! There are no recommended actions for you right now.', 'click' ) . '</span>';
                    echo '<a data-action="reset" id="reset" class="reset-all" href="#">'.esc_html__('Show All Recommended Actions','click').'</a>';
                }
                echo '</div>';
            }
        }
                        ...
                        if( !empty( $screenshot) ){
                            ...
                            if ( ! empty( $title ) ) {
                                ...
                                if ( ! empty( $download_link ) ) {
                                    echo '<a href="' . esc_url( $download_link ) . '" class="button button-primary install right">' . esc_html__( 'Download','click' ) . '</a>';
                                }
                                if ( ! empty( $preview_link ) ) {
                                    echo '<a class="button button-secondary preview right" target="_blank" href="' . $preview_link . '">' . esc_html__( 'Live Preview','click' ). '</a>';
                                }
                                echo '</div>';
                                echo '</div>';
                            }
        ...
        public function free_pro() {
            ...
            if ( ! empty( $free_pro ) ) {
                ...
                if ( ! empty( $free_pro ) && is_array( $free_pro ) ) {
                    ...
                    echo '<tr>';
                    echo '<th></th>';
                    echo '<th>' . esc_html__( 'Click','click' ) . '</th>';
                    echo '<th>' . esc_html__( 'Click Pro','click' ) . '</th>';
                    echo '</tr>';
                    echo '</thead>';
                    echo '<tbody>';
                    ...
                    echo '<td></td>';
                    echo '<td colspan="2"><a href="https://www.acmethemes.com/themes/click-pro/" target="_blank" class="button button-primary button-hero"> Click Pro</a></td>';
                    echo '</tr>';
        ...
        public function style_and_scripts( $hook_suffix ) {
            ...
            wp_enqueue_style( 'click-theme-info-css', get_template_directory_uri() . '/acmethemes/at-theme-info/css/at-theme-info.css' );

            if ( 'appearance_page_' . $this->theme_slug . '-info' === $hook_suffix ) {

                wp_enqueue_script( 'click-theme-info-js', get_template_directory_uri() . '/acmethemes/at-theme-info/js/at-theme-info.js', array( 'jquery' ) );
                ...
                $count = $this->count_recommended_actions();
                wp_localize_script( 'click-theme-info-js', 'at_theme_info_object', array(
...
$config = array(
    // Page title.
    'page_title'               => esc_html__( 'Click Info', 'click' ),

    // Menu name under Appearance.
    'menu_title'               => esc_html__( 'Click Info', 'click' ),

    // Main welcome title
    'info_title'         => sprintf( esc_html__( 'Welcome to %s - ', 'click' ), 'Click' ),

    // Main welcome content
    'info_content'       => sprintf( esc_html__( '%s is now installed and ready to use. We hope the following information will help you. If you want to ask any query or just want to say hello, you can always contact us. We hope you enjoy it! ', 'click' ), '<b>Click</b>' ),
    ...
    'quick_links'                    => array(
        'theme_url'  => array(
                'text' => __('Theme Details','click'),
                'url' => 'https://www.acmethemes.com/themes/click/'
        ),
        'demo_url'  => array(
            'text' => __('View Demo','click'),
            'url' => 'http://www.demo.acmethemes.com/click/'
        ),
        'pro_url'  => array(
            'text' => __('View Pro Version','click'),
            'url' => 'https://www.acmethemes.com/themes/click-pro/'
        ),
        'rate_url'  => array(
            'text' => __('Rate This Theme','click'),
            'url' => 'https://wordpress.org/support/theme/click/reviews/?filter=5'
        ),
    ),

    'tabs'                    => array(
        'getting_started'  => __( 'Getting Started', 'click' ),
        'recommended_actions' => __( 'Recommended Actions', 'click' ),
        'recommended_plugins' => __( 'Useful Plugins','click' ),
        'support'       => __( 'Support', 'click' ),
        'changelog'        => __( 'Changelog', 'click' ),
        'faq'        => __( 'FAQ', 'click' ),
        'free_pro'         => __( 'Free VS PRO', 'click' ),
    ),

    /*Getting started tab*/
    'getting_started' => array(
        'first' => array(
            'title' => esc_html__( 'Step 1 : Read full documentation','click' ),
            'desc' => esc_html__( 'Please check our full documentation for detailed information on how to Setup and Use Click.','click' ),
            'link_title' => esc_html__( 'Documentation','click' ),
            'link_url' => 'http://www.doc.acmethemes.com/click/',
            'is_button' => false,
            'recommended_actions' => false,
            'is_new_tab' => true
        ),
        'second' => array(
            'title' => esc_html__( 'Step 2 : Go to Customizer','click' ),
            'desc' => esc_html__( 'All Setting, Theme Options, Widgets and Menus are available via Customize screen. Have a quick look or start customization!','click' ),
            'link_title' => esc_html__( 'Go to Customizer','click' ),
        ...
        'third' => array(
	        'title' => esc_html__( 'Step 3 : Setting Featured Section ','click' ),
	        'desc' => esc_html__( 'Add Feature Image or Unlimited Slides on Feature Section. Please do the Step 3 before this.','click' ),
	        'link_title' => esc_html__( 'Featured Section','click' ),
	        'link_url' => esc_url( admin_url( 'customize.php' ) ).'?autofocus[panel]=click-feature-panel',
	        'is_button' => true,
	        'recommended_actions' => false,
	        'is_new_tab' => true
        )
    ...
    'recommended_actions'        => array(
        'demo-content' => array(
            'title' => esc_html__( 'Install Demo Setup','click' ),
            'desc' => sprintf( esc_html__( 'For the demo content installation, install the Acme Demo Setup plugin, download demo content from %1$s here %2$s. Then go to "Acme Demo Setup" and import demo. ', 'click' ), '<a target="_blank" href="http://www.doc.acmethemes.com/click/#DummyData" >','</a>' ),
            'id' => 'acme-demo-setup',
    ...
    'faq'      => array(
	    'first' => array (
		    'title' => esc_html__( 'Where to get a dedicated support ? ','click' ),
		    'icon' => 'dashicons dashicons-sos',
		    'desc' => esc_html__( 'Follow the theme dedicated support forum.','click' ),
		    'button_label' => esc_html__( 'Click here to get the support','click' ),
		    'button_link' => esc_url( 'https://www.acmethemes.com/supports' ),
		    'is_button' => true,
		    'is_new_tab' => true
	    ),
	    'second' => array (
		    'title' => esc_html__( 'Recent Updates of your Site ? ','click' ),
		    'icon' => 'dashicons dashicons-sos',
		    'desc' => esc_html__( 'Where i can get the recent updates of theme related updates ? ','click' ),
		    'button_label' => esc_html__( 'Visit our site blog page','click' ),
		    'button_link' => esc_url( 'https://www.acmethemes.com/blog/' ),
		    'is_button' => true,
		    'is_new_tab' => true
	    ),
	    'third' => array (
		    'title' => esc_html__( 'How to hide/change footer text Proudly powered by WordPress | Theme: Click by AcmeThemes. ? ','click' ),
		    'icon' => 'dashicons dashicons-sos',
		    'button_label' => esc_html__( 'Visit this blog post to know how to change or remove powered by text.','click' ),
    ...
    'support_content'      => array(
	    'first' => array (
		    'title' => esc_html__( 'Contact Support','click' ),
		    'icon' => 'dashicons dashicons-sos',
		    'desc' => esc_html__( 'Got theme support question or found bug? Best place to ask your query is our dedicated Support forum.','click' ),
		    'button_label' => esc_html__( 'Contact Support','click' ),
		    'button_link' => esc_url( 'https://www.acmethemes.com/supports/forum/click/' ),
		    'is_button' => true,
		    'is_new_tab' => true
	    ),
	    'second' => array(
		    'title' => esc_html__( 'Documentation','click' ),
		    'icon' => 'dashicons dashicons-book-alt',
		    'desc' => esc_html__( 'Please check our full documentation for detailed information on how to Setup and Use Click.','click' ),
		    'button_label' => esc_html__( 'Read full documentation','click' ),
		    'button_link' => 'http://www.doc.acmethemes.com/click/',
		    'is_button' => false,
		    'is_new_tab' => true
	    ),
	    'third' => array(
		    'title' => esc_html__( 'Need more features?','click' ),
		    'icon' => 'dashicons dashicons-screenoptions',
		    'desc' => esc_html__( 'Upgrade to PRO version for more exciting features and Priority Support.','click' ),
		    'button_label' => esc_html__( 'View Pro Version','click' ),
		    'button_link' => 'https://www.acmethemes.com/themes/click-pro/',
		    'is_button' => true,
		    'is_new_tab' => true
	    ),
	    'fourth' => array(
		    'title' => esc_html__( 'Got sales related question?','click' ),
		    'icon' => 'dashicons dashicons-cart',
		    'desc' => esc_html__( "Have any query before purchase, you are more than welcome to ask.",'click' ),
		    'button_label' => esc_html__( 'Pre-sale Question?','click' ),
		    'button_link' => 'https://www.acmethemes.com/pre-sale-question/',
		    'is_button' => true,
		    'is_new_tab' => true
	    ),
	    'fifth' => array(
		    'title' => esc_html__( 'Customization Request','click' ),
		    'icon' => 'dashicons dashicons-hammer',
		    'desc' => esc_html__( 'Needed any customization for the theme, you can request from here.','click' ),
		    'button_label' => esc_html__( 'Customization Request','click' ),
    ...
    'free_pro' => array(
	    array(
		    'title'=> __( 'Custom Widgets', 'click' ),
		    'desc' => __( 'All added custom widgets are ready for page builder plugin', 'click' ),
		    'free' => __('1','click'),
		    'pro'  => __('8+ Available','click'),
	    ),
	    array(
		    'title'       => __( 'Widget Options', 'click' ),
		    'desc' => __( 'Every widgets have multiple options to customize section', 'click' ),
		    'free'  => __('Not Available','click'),
		    'pro'   => __('Multiple Options','click'),
	    ),
	    array(
		    'title'       => __( 'Google Fonts', 'click' ),
		    'desc' => __( 'Google fonts options for changing the overall site fonts ', 'click' ),
		    'free'  => 'yes',
		    'pro'   => __('100 +','click'),
	    ),
	    array(
		    'title'       => __( 'Color Options', 'click' ),
		    'desc' => __( 'Color options for changing overall site colors, Contat us', 'click' ),
		    'free'  => __('2','click'),
		    'pro'   => __('Unlimited','click'),
	    ),
	    array(
		    'title'       => __( 'Header Options', 'click' ),
		    'desc' => __( 'Customze the header section as your need.', 'click' ),
		    'free'  => __('Limited','click'),
		    'pro'   => __('Multiple Options','click'),
	    ),
	    array(
		    'title'       => __( 'Menu Options', 'click' ),
		    'desc' => __( 'Lots of menu options are available to manage the menu section. Appearance > Menus', 'click' ),
		    'free'  => __('Limited','click'),
		    'pro'   => __('Multiple Options','click'),
	    ),
	    array(
		    'title'       => __( 'Sticky Menu Options', 'click' ),
		    'desc' => __( 'Enable Sticky menu on your site.', 'click' ),
		    'free'  => 'no',
		    'pro'   => 'yes'
	    ),
	    array(
		    'title'       => __( 'Featured Selection', 'click' ),
		    'desc' => __( 'This is slider section. Check every options there','click' ),
		    'free'  => __('Parent/Child Page','click'),
		    'pro'   => __('Page, Post, Category, And Custom','click'),
	    ),
	    array(
		    'title'       => __( 'Featured Section Options', 'click' ),
		    'desc' => __( 'Options to manage the sliders', 'click' ),
		    'free'  => __('Limited','click'),
		    'pro'   => __('Multiple Options','click'),
	    ),
	    array(
		    'title'       => __( 'Sticky Sidebar', 'click' ),
		    'desc' => __( 'Enable/Disable sticky sidebars', 'click' ),
		    'free'  => 'yes',
		    'pro'   => 'yes',
	    ),
	    array(
		    'title'       => __( 'Blog/Archive Options', 'click' ),
		    'desc' => __( 'Various Options are available for the blog and archive page management. Check on Appearance > Customize > Layout/Design Options.', 'click' ),
		    'free'  => __('Limited','click'),
		    'pro'   => __('Manage Every Entity','click'),
	    ),
	    array(
		    'title'       => __( 'Footer Options', 'click' ),
		    'desc' => __( 'Manage the every entity related to footer section.', 'click' ),
		    'free'  => __('Limited','click'),
		    'pro'   => __('Multiple Options','click'),
	    ),
	    array(
		    'title'       => __( 'Social Options', 'click' ),
		    'desc' => __( 'Options to manage the social icons like facebook, instagram, linkedin etc.', 'click' ),
		    'free'  => __('Limited','click'),
		    'pro'   => __('Multiple Options','click'),
	    ),
	    array(
		    'title'       => __( 'Theme Credit Link', 'click' ),
		    'desc' => __( 'Options to disable to Powered by text on footer.', 'click' ),
		    'free'  => __('No Options To Disable','click'),
		    'pro'   => __('Enable/Disable','click'),
	    ),
	    array(
		    'title'       => __( 'Own Credit Link', 'click' ),
		    'desc' => __( 'Write your own credit text.', 'click' ),
		    'free'  => 'no',
		    'pro'   => __('Your Own Credit Text','click'),
	    ),
	    array(
		    'title'       => __( 'Social Icons', 'click' ),
		    'desc' => __( 'Enter the URL for social icons. ', 'click' ),
		    'free'  => __('4 +','click'),
		    'pro'   => __('Available Mostly Used','click'),
	    ),
	    array(
		    'title'       => __( 'Intro Loader Options', 'click' ),
		    'desc' => __( 'It will load before the site loaded completely.', 'click' ),
		    'free'  => 'no',
		    'pro'   => __('Multiple Options','click'),
	    ),
	    array(
		    'title'       => __( 'Related Post Options', 'click' ),
		    'desc' => __( 'Related post will appear on single post of each blog article', 'click' ),
		    'free'   => 'no',
		    'pro'   => __('Multiple Options','click'),
	    ),
	    array(
		    'title'       => __( 'Author Info Options', 'click' ),
		    'desc' => __( 'This will appear on single page and archive page of that author.', 'click' ),
		    'free'   => 'no',
		    'pro'   => 'yes'
	    ),
	    array(
		    'title'       => __( 'Feature Image Options', 'click' ),
		    'desc' => __( 'Featured image options for the single page and post.', 'click' ),
		    'free'  => 'yes',
		    'pro'   => __('Manage As Your Need','click'),
	    ),
	    array(
		    'title'       => __( 'Home Page Layout', 'click' ),
		    'desc' => __( 'Change your home page layout with available options', 'click' ),
		    'free'  => __('Masonry','click'),
		    'pro'   => __('Multiple Layout Available','click'),
	    ),
	    array(
		    'title'       => __( 'Navigation Options', 'click' ),
		    'desc' => __( 'Post Navigation means, previous post and next post. Check on single post.', 'click' ),
		    'free'   => __('Default','click'),
		    'pro'   => __('Advanced','click'),
	    ),
	    array(
		    'title'       => __( 'Post Comment Options', 'click' ),
		    'desc' => __( 'Manage the comment section on single post.', 'click' ),
		    'free'  => 'no',
		    'pro'   => __('Options Available','click'),
	    ),
	    array(
		    'title'       => __( 'Show/Hide Comment', 'click' ),
		    'desc' => __( 'Options to show hide comments on page and post.', 'click' ),
		    'free'  => 'no',
		    'pro'   => 'yes',
	    ),
	    array(
		    'title'       => __( 'Breadcrumb', 'click' ),
		    'desc' => __( 'Advanced options for manage the Breadcrumb.', 'click' ),
		    'free'  => 'yes',
		    'pro'   => __('Advanced','click'),
	    ),
	    array(
		    'title'       => __( 'Video Post On Featured', 'click' ),
		    'desc' => __( 'Create your video post easily from Youtube and Vimeo video.', 'click' ),
		    'free'  => 'no',
		    'pro'   => __('Add Own Video On Featured','click'),
	    ),
	    array(
		    'title'=> __( 'Single Post Options', 'click' ),
		    'desc' => __( 'Manage every entity of single post.', 'click' ),
		    'free' => __('Limited','click'),
		    'pro'  => __('Multiple Options','click'),
	    ),
	    array(
		    'title'=> __( 'Featured Image Size', 'click' ),
		    'desc' => __( 'Select the size of the featured image on the single post.', 'click' ),
		    'free' => 'yes',
		    'pro'  => 'yes',
	    ),
	    array(
		    'title'=> __( 'Image Size Options', 'click' ),
		    'desc' => __( 'Make the image size as your need on blog/archive page.', 'click' ),
		    'free' => 'yes',
		    'pro'  => 'yes',
	    ),
	    array(
		    'title'=> __( 'Advertisement Options', 'click' ),
		    'desc' => __( 'Can we advertise with this theme?', 'click' ),
		    'free' => 'no',
		    'pro'  => 'yes',
	    ),
	    array(
		    'title'=> __( 'Advertisement Widget', 'click' ),
		    'desc' => __( 'Is custom widget available for Advertisement?', 'click' ),
		    'free' => 'No',
		    'pro'  => 'yes',
	    ),
	    array(
		    'title'=> __( 'Sidebars', 'click' ),
		    'desc' => __( 'Global and Individual sidebars are available. You can easily orveride it from single page/post.', 'click' ),
		    'free' => 'yes',
		    'pro'  => __('Global And Individuals', 'click'),
	    ),
	    array(
		    'title'=> __( 'Infinite Scroll Features', 'click' ),
		    'desc' => __( 'Is theme have default infinite scroll ?', 'click' ),
		    'free' => 'no',
		    'pro'  => 'yes',
	    ),
	    array(
		    'title'=> __( 'Infinite Scroll', 'click' ),
		    'desc' => __( 'Use Jetpack Infinite Scroll on Blog Page', 'click' ),
		    'free' => 'yes',
		    'pro'  => 'yes',
	    ),
	    array(
		    'title'=> __( 'WooCommerce', 'click' ),
		    'desc' => __( 'Create WooCommerce shop easily with the help of WooCommerce plugin.', 'click' ),
		    'free' => 'yes',
		    'pro'  => 'yes',
	    ),
	    array(
		    'title'       => __( 'Translation', 'click' ),
		    'desc' => __( 'Both theme are translation ready.', 'click' ),
		    'free'  => 'yes',
		    'pro'   => 'yes',
	    ),
	    array(
		    'title'       => __( 'SEO', 'click' ),
		    'desc' => __( 'Developed with high skilled SEO tools.', 'click' ),
		    'free'  => 'yes',
		    'pro'   => 'yes',
	    ),
	    array(
		    'title'       => __( 'Widget Area On Footer', 'click' ),
		    'desc' => __( 'Change the number of footers on footer area as you need.', 'click' ),
		    'free'  => __('3 ', 'click'),
		    'pro'   => __('Choose On Need', 'click'),
	    ),
	    array(
		    'title'       => __( 'Support Forum', 'click' ),
		    'desc' => __( 'Highly dedicated support team are assigned for your help. Try this today.', 'click' ),
		    'free'  => __('Second', 'click'),
		    'pro'   => __('Dedicated With High Priority', 'click'),
	    )
    )
);
return new Click_Theme_Info( $config );